...
def write_manifest(output_path, target_manifest, ensure_ascii: bool = False):
    """
    ...
    Args:
        ...
        ensure_ascii (bool): If is True, meaning the output is guaranteed to have all incoming
...
parser.add_argument("--results_dir", type=str, default=None, help='path to save the overall results')
...
def write_evaluation(results: dict, results_dir: str = None):
    ...
    if results_dir is not None:
        ...
        with open(os.path.join(results_dir, 'results.txt'), 'a') as f:
            ...
            f.write(f'{df}\n')
            f.write('--------------------------------\n')
        print(f'\nUpdated eval results to {os.path.join(results_dir, "results.txt")}')
...
def main():
    ...
    # Write to csv
    write_evaluation(eval_results, results_dir=args.results_dir)
