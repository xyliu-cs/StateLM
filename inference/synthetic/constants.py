...
def string_match_all_strip_comma(preds, refs):
    score = sum([sum([1.0 if r.lower() in pred.lower().replace(',', '') else 0.0 for r in ref]) / len(ref) for pred, ref in zip(preds, refs)]) / len(preds) * 100
    return round(score, 2)
...
TASKS = {
    ...
    # },
    'niah': {
        'metric_fn': string_match_all_strip_comma,
    },
    'variable_tracking': {
        'metric_fn': string_match_all,
    },
